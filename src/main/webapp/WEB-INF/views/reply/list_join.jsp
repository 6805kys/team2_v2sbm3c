    ...
    <table class="table table-striped" style='width: 100%;'>
      ...
      <tbody>
        <c:forEach var="replyMemberVO" items="${list }">
          ...
          <c:set var="content_no" value="${replyMemberVO.content_no }" />
          <c:set var="memberno" value="${replyMemberVO.memberno }" />
          <c:set var="id" value="${replyMemberVO.id }" />
          <c:set var="reply_content" value="${replyMemberVO.reply_content }" />
          <c:set var="reply_crtime" value="${replyMemberVO.reply_crtime }" />
          ...
          <tr style='height: 50px;'> 
            ...
            <td style='text-align: center; vertical-align: middle;'>
              <A href='../brcontent/read.do?content_no=${content_no }'>${content_no}</A>
            </td>
            <td style='text-align: center; vertical-align: middle;'>
              <A href='../member/read.do?memberno=${memberno }'>${id}</A>
            </td>
            <td style='text-align: left; vertical-align: middle;'>${reply_content}</td>
            <td style='text-align: center; vertical-align: middle;'>
              ${reply_crtime.substring(0, 10)}
            </td>
            <td style='text-align: center; vertical-align: middle;'>
